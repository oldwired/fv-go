...
To cut a release, tag from main and push:
...
tag push and creates a **draft** GitHub Release for that tag with
auto-generated notes (commit log since the previous tag + new
contributors). A CycloneDX SBOM is generated by `cyclonedx-gomod mod`
and attached to the draft as `fv-go-<tag>.cdx.json`. Nothing is
visible to consumers until you review the draft on the Releases page
and click **Publish release**.

Consumers retrieve a published release's SBOM with:

```bash
gh release download v0.4.0 -p '*.cdx.json'
```
